...
import org.apache.spark.util.sgx.TrustedSGXVM
...
class RDDSuiteSGX extends SparkFunSuite with SharedSparkContext {

  private var tempDir: File = _
  @volatile private var _sgxUntrustedCommThreadRun: Boolean = true
  @volatile private var _sgxTrustedCommThreadRun: Boolean = true

  val cleanShmScript = "/secureVmClean.sh"
  val cleanShmScriptPath = getClass.getResource(cleanShmScript).getPath
  ...
  override def beforeAll(): Unit = {
    // Cleaning sharedMemory leftovers
    val shmClean = Process(Seq("sh", cleanShmScriptPath)).!!
    logInfo(s"Cleaning SharedMemory: ${shmClean}")

    // Worker side of SHM (untrusted)
    setEnv("SGX_ENABLED", "true")
    setEnv("IS_ENCLAVE", "false")
    setEnv("DEBUG_IS_ENCLAVE_REAL", "false")
    setEnv("SGXLKL_SHMEM_FILE", "sgx-lkl-shmem")

    logDebug("Running untrusted Worker")
    Completor.submit(new Waiter())
    val sgxUntrustedCommRunner = new Runnable {
      override def run(): Unit = {
        while (_sgxUntrustedCommThreadRun) {
          Completor.submit(new SgxMainRunner(SgxFactory.acceptCommunicator()))
        }
      }
    }
    new Thread(sgxUntrustedCommRunner).start()



    logDebug("Running trusted Worker on separate VM")
    val sgxTrustedCommRunner = new Runnable {
      override def run(): Unit = {
        Thread.sleep(2000)
        val trustedVMproc = Process(Seq("sh", TrustedSGXVM.launchScriptPath)).!!
        logInfo(s"Running trusted Worker: ${trustedVMproc}")
  ...
  override def afterAll(): Unit = {
    try {
      ...
      _sgxUntrustedCommThreadRun = false
...
//    }
  ...
  test("SparkContext.union") {
    ...
    val result = sc.union(nums).collect().toList
